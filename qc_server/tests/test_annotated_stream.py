...
def test_annotated_mjpeg_counter_drives_count_and_skips_crop_sink(monkeypatch):
    monkeypatch.setattr(
        ann,
        "detect",
        lambda frame, conf, model_path: [Detection(0, 0, 5, 5, "x", 0.9)],
    ...
    def counter(orig, dets, scale):
        counter_calls.append(len(dets))
        return 42
    ...
    list(
        ann.annotated_mjpeg(
            ...
            0.5,
            "m.pt",
            lambda count, fps: stats.append(count),
            max_width=960,
            max_fps=0,
            crop_sink=lambda f, d, s: sink_calls.append(1),
            counter=counter,
        )
    )
    assert counter_calls == [1]
    assert sink_calls == []
    assert stats and stats[0] == 42
