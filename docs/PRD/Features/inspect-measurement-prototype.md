...
Canvas controls mengikuti QC Studio: wheel atau tombol zoom pada range `50%–500%`, drag untuk pan, dan `Reset` untuk kembali ke fit awal. Kontrol ini tersedia sebelum process agar calibration presisi. Preview dan processed canvas memakai satu frame rasio-source yang fit ke viewport; image dan SVG memakai transform yang sama, tanpa independent SVG fitting, sehingga auto-detected jig point tidak bergeser secara visual. Saat menggambar, crosshair dash opacity rendah membantu alignment. Setelah process, label `REF X` berada di midpoint sedikit di atas garis X dan `REF Y` berada di midpoint di sisi kiri garis Y, masing-masing dengan halo canvas supaya tidak menumpuk pada titik awal, garis reference, atau label measurement. Mobile Camera menampilkan crosshair tengah sebagai guide positioning dan tidak menulis overlay ke capture. `Reset workspace` membersihkan media staged dan state lokal tanpa menghapus History/Audit. Toolbar overlay menyediakan toggle `Deteksi`, `Hasil`, dan `Kalibrasi`; setelah Evaluate, detector disembunyikan default sehingga hanya evidence measurement yang terlihat. Label hasil memakai text 15px dengan leader line ke midpoint/center geometry dan status `PASS`/`FAIL`/`REVIEW`. Lighting health warning berbasis baseline profile station adalah improvement Stage 00 berikutnya; warning tidak memblok capture.
