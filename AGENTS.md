...
[Camera List] → [Select Camera] → [Start Stream]
    ...
    → [QC Studio: View-only review/export OR Edit mode manual add/delete/relabel + zoom/pan + mark reviewed + optional re-run/reset]
...
| QC Studio | **Functional** | None for live batches | Pending raw image list/canvas, image delete, view-only default, persisted edit mode for manual polygon add/delete/relabel, re-run/reset controls, batch polling/result/review/sign-off API wired; configured class colors; `sam3_prompt` strategy implemented for configured SAM 3 weights |
...
**Latest version**: [Unreleased] - 2026-07-01 (QC Studio Manual Defect Editing; Audit Report PDF Defect Crops; Media Detection Multi-Image Upload; QC Studio Batch Export).
