...
- **Measurement Studio layout**: Desktop shell follows QC Studio fixed-height flex positioning with fixed left/canvas/right rails; Input/Calibration, History, and Measurement Items each own bounded scroll regions. Canvas uses shared image/SVG coordinates, QC Studio-style zoom/pan, restrained 3px edge strokes with contrast halos, and visible measurement labels. Narrow screens fall back to page scroll for stacked panels.
