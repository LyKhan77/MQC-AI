...
- **Inspect Measurement**: Measurement Studio accepts one uploaded image or one-shot trigger from a registered Live Camera. Inspector enters manual reference scale, reviews OpenCV LSD/Hough edge candidates, selects linear items, sets nominal/tolerance, evaluates `PASS`/`FAIL`/`REVIEW`, then saves named runs to History and Audit. Prototype UI is linear-edge focused; angle/hole math remains kernel-ready for later UI extension.
- **Measurement Studio layout**: Desktop shell follows QC Studio fixed-height behavior; page/header/canvas stay fixed while History and Measurement Items scroll independently. Mobile Camera remains a follow-up after layout feedback.
...
[Camera List] → [Select Camera] → [Start Stream]
    ...
    → [Optional Inspect Measurement: upload/Live Camera trigger → manual calibration → OpenCV edge candidates → tolerance/evaluate → save History/Audit]
...
| Inspect Measurement | **Functional** (prototype) | None | `/measurement` supports image upload and registered Live Camera trigger, manual scale calibration, OpenCV LSD/Hough candidate overlay, linear tolerance/evaluate, save/reopen/delete History, and measurement audit events; M6 physical accuracy validation pending |
