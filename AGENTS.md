...
- **Quantity Detection** (Phase 1: image): dedicated "Quantity" sidebar group. Quantity Detection page runs the configured Quantity Detection model against one or more uploaded images, accumulates a session cumulative total + per-class counts, computes a Pass/Fail verdict against an optional expected total + tolerance, and saves the result (persisted record + `QUANTITY_CHECK` audit log entry). Quantity History page lists saved checks with search/filter and CSV export.
...
- Quantity Detection (Phase 1: image) endpoints: `POST /api/quantity/detect/image` reuses `object_detection.detect()`/`serialize_detections()` with the dedicated `quantity_model`/`quantity_confidence_threshold` settings (409 if no quantity model configured), and `POST/GET/GET-by-id /api/quantity/checks` persist/list/retrieve saved `QuantityCheck` records (total/per-class counts, expected total, tolerance, verdict).
...
│       │   ├── quantity.js          # Quantity detect-image/checks API
...
│       │   ├── useQuantityHistory.js # Saved quantity-check list (live API)
...
│       │   ├── QuantityDetection.vue # Quantity count/verify (session accumulation + Pass/Fail verdict)
│       │   ├── QuantityHistory.vue  # Saved quantity checks (search/filter + CSV export)
...
│   │   │   ├── quantity.py          # Quantity detect-image + checks save/list/detail
│   │   │   └── settings.py
│   │   └── services/
...
| Quantity Detection | **Functional** (Phase 1: image) | None | Multi-image upload via `/api/quantity/detect/image`, session-cumulative total + per-class counts, expected-total + tolerance Pass/Fail verdict, Save via `/api/quantity/checks` + `QUANTITY_CHECK` audit log wired; requires a Quantity Detection model configured in Settings |
| Quantity History | **Functional** | None | Live `/api/quantity/checks` list, search/filter, and CSV export wired |

### Detailed Log

See [`CHANGELOG.md`](./CHANGELOG.md) for the comprehensive, per-feature change log with Current Codebase State tables.

**Latest version**: [Unreleased] - 2026-07-02 (Quantity Detection Phase 1 (Image); QC Studio Vertex Reshape Tool; Config Storage Path Anchoring; QC Studio Edit Dock Cancel and Labels; QC Studio SAM Click-to-Segment; QC Studio Edit-Mode UX; QC Studio Manual Defect Editing; Audit Report PDF Defect Crops; Media Detection Multi-Image Upload; QC Studio Batch Export).
