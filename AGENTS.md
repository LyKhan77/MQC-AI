...
- **Measurement Studio layout**: Desktop shell follows QC Studio fixed-height behavior with wider input/result panels; page/header/canvas stay fixed while History and Measurement Items scroll independently. Canvas uses shared image/SVG coordinates, QC Studio-style zoom/pan, high-contrast edge halos, and visible measurement labels. Narrow screens fall back to page scroll for stacked panels.
