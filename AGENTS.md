...
- **Inspect Measurement**: Measurement Studio accepts Image, registered Live Camera trigger, or HTTPS Mobile Camera capture. Inspector draws a reference line, selects explicit task/view (`linear_dimension`, `inclination`, profile `bend_angle`, hole diameter/pitch/edge/center-to-edge), sets per-item nominal/tolerance, evaluates `PASS`/`FAIL`/`REVIEW`, then saves geometry, task metadata, History, and Audit. OpenCV uses LSD/Hough line candidates plus Hough circle candidate + contour/ellipse refinement; thickness remains profile-only and unimplemented until profile metrology validation.
...
│       │   ├── measurements.js      # Inspect Measurement process/save/history API
...
│       │   ├── MeasurementStudio.vue # Inspect Measurement task/view/calibration/hole UI
...
│   │   │   ├── measurements.py      # Inspect Measurement process/save/history endpoints
│   │   │   ├── images.py
...
[Camera List] → [Select Camera] → [Start Stream]
    ...
    → [Optional Inspect Measurement: upload/Live Camera trigger/Mobile Camera capture → manual calibration → OpenCV edge candidates → tolerance/evaluate → save History/Audit]
...
### Status: SAM Click-to-Segment Implemented · Inspect Measurement Proper P1-P3 Implemented · Backend M0-M3 Implemented
...
Inspect Measurement is wired at `/measurement`: Image upload, Live Camera `/api/measurements/capture`, and Mobile Camera client capture all stage a frame before `POST /api/measurements/process`; the server calibrates the drawn reference line, extracts task-specific LSD/Hough line or Hough circle candidates, and returns `REVIEW` for invalid calibration, unsupported view, or unusable candidates. Measurement Studio overlays line/circle geometry, supports linear/inclination/bend/hole task selection, per-item tolerance/evaluation, `MeasurementRun` persistence with task/view/method metadata, History, and measurement audit actions. Physical accuracy, repeatability, station matrix/homography runtime, multi-view sessions, drawing recipes, and thickness profile metrology remain future gates.
...
| Inspect Measurement | **Functional** (proper P1-P3) | None | `/measurement` supports Image/Live/Mobile sources, drawn reference-line calibration, task/view selection, LSD/Hough line candidates, Hough circle + ellipse refinement, linear/inclination/bend/hole geometry, per-item tolerance/evaluate, save/reopen/delete History with task metadata, and measurement audit events; station accuracy/profile metrology remains pending |
