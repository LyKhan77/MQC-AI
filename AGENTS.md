...
- **Direct Inspection**: Quality Control submenu for on-demand QC from image upload, server-camera frame, or mobile HTTPS camera capture, with full-frame/auto-crop modes, optional auto-crop debug overlay (AUTO-CROP/FULL FRAME FALLBACK), defect strategy preview, annotated capture stack, remove, and Send to QC Studio done-batch handoff with persisted defects.
