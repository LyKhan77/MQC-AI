...
- **Quantity Detection** (Phase 1: image): dedicated "Quantity" sidebar group with distinct count/history icons. Quantity Detection page runs the configured Quantity Detection model against one or more uploaded images, overlays returned detection boxes on each image, shows per-image count badges, accumulates a session cumulative total + per-class counts, computes a Pass/Fail verdict against a labeled expected total + tolerance result band, and saves the result (persisted record + per-image `inputs` + `QUANTITY_CHECK` audit log entry). Quantity History page lists saved checks with search/filter, CSV export, Inspect detail dialog, and Delete confirmation.
...
│       │   ├── QuantityDetection.vue # Quantity count/verify (annotated boxes + result band)
│       │   ├── QuantityHistory.vue  # Saved quantity checks (search/filter + CSV export + inspect/delete)
