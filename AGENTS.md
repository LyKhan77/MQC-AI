...
- **Inspect Measurement**: Measurement Studio accepts multiple Image files, registered Live Camera trigger, or HTTPS Mobile Camera capture in one manually named component session. Inspector stages dynamic views, selects one view, chooses pose (`TOP_FACE`, `REVERSE_FACE`, `PROFILE_FACE`, `CUSTOM_FACE`) and optional Global/Detail profile, draws calibration, explicitly processes, sets per-item nominal/tolerance, evaluates `PASS`/`FAIL`/`REVIEW`, saves/reopens/deletes views, and completes only after saved views. OpenCV uses LSD/Hough line candidates plus Hough circle candidate + contour/ellipse refinement; thickness/bend remain profile-metrology contracts and physical station accuracy remains pending.
...
### Status: SAM Click-to-Segment Implemented · Inspect Measurement Multi-View Software Slice Implemented · Backend M0-M3 Implemented
...
Inspect Measurement is wired at `/measurement`: one manually named session accepts multiple Image/Live/Mobile staged views; selected view calibration and explicit `POST /api/measurements/process` carry pose/profile metadata. The server calibrates the drawn reference line, extracts task-specific LSD/Hough line or Hough circle candidates, validates Global/Detail capability and camera/resolution binding, and returns `REVIEW` for invalid calibration, unsupported pose, profile mismatch, or unusable candidates. Measurement Studio persists selected views through session APIs, restores them in History, and uses backend Audit events. Physical accuracy, repeatability, station matrix/homography runtime, drawing recipes, and profile metrology remain future gates.
...
| Inspect Measurement | **Functional** (multi-view software slice) | None | `/measurement` supports manual sessions, multiple Image/Live/Mobile views, selected-view capture -> calibration -> explicit process, pose and Global/Detail profile guidance, LSD/Hough/circle geometry, per-item tolerance/evaluate, save/reopen/delete view/session History, completion guard, and backend Audit; station accuracy/profile metrology/drawing recipes remain pending |
