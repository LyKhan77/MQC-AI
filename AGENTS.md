...
- **Inspect Measurement**: Measurement Studio accepts multiple Image files, registered Live Camera trigger, or HTTPS Mobile Camera capture in one manually named component session. Inspector stages dynamic views, selects one view, chooses dimension/corner/bend task checks, pose (`TOP_FACE`, `REVERSE_FACE`, `PROFILE_FACE`, `CUSTOM_FACE`) and optional Global/Detail profile, calibrates X/Y manually or detects four green jig points to place X/Y lines (physical lengths remain manual), explicitly processes, selects logical edge/corner/bend geometry, sets per-item nominal/tolerance, evaluates `PASS`/`FAIL`/`REVIEW`, saves/reopens/deletes views, and completes only after saved views. Every Staged View can download its original image to the browser client. Preview and processed measurement share one viewport-fitted image/SVG frame and transform, so auto-detected jig points remain at returned source-frame coordinates before and after Process. Mobile preview has a non-captured center guide; Reset workspace clears local staged state; dummy manual-edge creation is removed. Overlay controls expose detected geometry, evaluated measurements, and calibration references separately; Evaluate hides detector geometry by default and labels evaluated evidence with status plus a leader line. OpenCV runs refined LSD and Hough candidates on raw grayscale plus CLAHE adaptive-contrast variants, with median-adaptive Canny thresholds, then applies support-bounded logical-edge grouping/fit/dedup. If detector lines miss a contour boundary, a source-labelled `contour_fallback` edge can be returned at fixed `0.45` confidence, which always evaluates as `REVIEW`. It also supports contour arc fitting with a `1.5 mm` prototype minimum radius, bend intersection geometry, and Hough circle candidate + contour/ellipse refinement. Calibration source is explicit: component-edge demo stays `REVIEW`; independent artifact can be verdict-eligible. Lighting health warning baseline and physical station accuracy/profile metrology remain pending. Geometry kernel v2 (I7): corner radius uses a Taubin circle fit with physical-arclength cluster windows, tangent-point exclusion, and a 30–60° `arc_coverage_low` ladder (≥60° clean); straight-edge endpoints snap to perpendicular fitted-line intersections (virtual corner / mold line) with `endpoint_sources` diagnostics; a new `outer_dimension` task returns `overall_candidates` Overall X/Y bounding-box extents in the component axis frame with support-validated contour extremes, exposed in Studio as an "Overall dimension (X/Y)" check with Overall X/Y candidate cards, dashed extent overlays, and bilingual labels.
- **Measurement Studio layout**: Desktop shell starts below the application TopBar and follows QC Studio fixed-height flex positioning without an in-page heading, outer padding, or modal-like container; fixed rails use the same left `280px` / center / right `320px` structure. Input/Calibration, History, and Measurement Items each own bounded scroll regions. Source/process/status controls float over the canvas, which uses shared image/SVG coordinates, QC Studio-style zoom/pan, restrained 3px edge strokes with contrast halos, and visible measurement labels. Processed `REF X` sits above its axis midpoint, while `REF Y` sits left of its axis midpoint with a canvas halo, preventing first-point stacking and line overlap. Narrow screens fall back to page scroll for stacked panels.
...
│       │   ├── measurements.js      # Inspect Measurement process + profile/session/view API
...
│       ├── router/
│       │   └── index.js             # 11 named routes
│       ├── assets/
│       │   └── locales/
│       │       ├── id.js            # Indonesian translations (~300 strings)
│       │       └── en.js            # English translations
│       ├── components/
│       │   ├── AppSidebar.vue       # Collapsible grouped nav (11 pages, icon components, keyboard nav)
...
│       │   ├── MeasurementStudio.vue # Inspect Measurement multi-view/task/pose/profile UI
...
│       └── utils/
│           ├── measurement.js       # Pure geometry/tolerance/evaluation helpers
...
│   │   │   ├── measurements.py      # Inspect Measurement process + profile/session/view endpoints
│   │   │   ├── images.py
...
│   │       ├── measurement.py       # OpenCV calibration, LSD/Hough/circle candidates, geometry/evaluation
...
│   ├── PRD/Features/         # Per-feature PRDs/specifications and milestone trackers
│   └── superpowers/
...
[Camera List] → [Select Camera] → [Start Stream]
    ...
    → [Optional Inspect Measurement: name session → stage Image/Live/Mobile views → select view → calibration → explicit Process → OpenCV candidates → tolerance/evaluate → save view/complete → History/Audit]
...
### Status: SAM Click-to-Segment Implemented · Inspect Measurement I2-I3+I7 Geometry Implemented · Backend M0-M3 Implemented
...
Inspect Measurement is wired at `/measurement`: one manually named session accepts multiple Image/Live/Mobile staged views; selected view task checks, X/Y calibration, and explicit `POST /api/measurements/process` carry pose/profile metadata. The server extracts LSD/Hough candidates, groups/fits/deduplicates support-bounded logical edges, fits rounded-corner arcs with a minimum-radius gate, derives finite-edge bend angles, and returns calibration/task-specific `REVIEW` reasons. Geometry kernel v2 (I7, branch `feat/inspect-measurement-geometry-v2`) adds Taubin corner fitting with arclength windows + tangent exclusion, virtual-corner endpoint snapping, and the `outer_dimension` Overall X/Y bounding-box task. Measurement Studio supports dimension/corner/bend/overall multi-select, candidate cards, explicit calibration-source selection, per-item tolerance, save/reopen/delete, History, and backend Audit events. Physical accuracy, repeatability, station matrix/homography runtime, drawing recipes, and profile metrology remain future gates.
...
| Inspect Measurement | **Functional** (I2-I3 software slice) | None | `/measurement` supports manual sessions, multiple Image/Live/Mobile views, dimension/corner/bend task selection, dual-axis calibration, logical LSD/Hough edges, corner arc and bend geometry, per-item tolerance/evaluate, save/reopen/delete view/session History, completion guard, and backend Audit; station accuracy/profile metrology/drawing recipes remain pending |
