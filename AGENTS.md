...
- **Inspect Measurement**: Measurement Studio accepts one uploaded image, one-shot trigger from a registered Live Camera, or one-shot HTTPS Mobile Camera capture. Inspector enters manual reference scale, reviews OpenCV LSD/Hough edge candidates, selects linear items, sets nominal/tolerance, evaluates `PASS`/`FAIL`/`REVIEW`, then saves named runs to History and Audit. Prototype UI is linear-edge focused; angle/hole math remains kernel-ready for later UI extension.
...
│       │   ├── MeasurementStudio.vue # Inspect Measurement image/live/tolerance/history UI
...
│   │       ├── measurement.py       # OpenCV calibration, LSD/Hough candidates, geometry/evaluation
...
### Status: SAM Click-to-Segment Implemented · Inspect Measurement Prototype Implemented · Backend M0-M3 Implemented
...
Inspect Measurement prototype is wired at `/measurement`: image upload, registered Live Camera trigger, and HTTPS Mobile Camera capture share `POST /api/measurements/process`; the server stores a temporary frame, calibrates a manual reference scale, extracts line candidates with OpenCV LSD plus Hough fallback, and returns a `REVIEW` readiness gate when calibration or edge confidence is insufficient. Measurement Studio overlays candidates/reference geometry, supports linear item tolerance/evaluation, saves `MeasurementRun` records, lists/reopens/deletes History, and records measurement audit actions. Physical accuracy, repeatability, and station calibration remain M6 validation work.
...
| Inspect Measurement | **Functional** (prototype) | None | `/measurement` supports image upload, registered Live Camera trigger, HTTPS Mobile Camera capture, manual scale calibration, OpenCV LSD/Hough candidate overlay, linear tolerance/evaluate, save/reopen/delete History, and measurement audit events; M6 physical accuracy validation pending |
