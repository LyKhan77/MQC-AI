...
- **Settings**: API-backed camera CRUD, grouped "Models" section (object-detection model switcher `active_model`, QC/segmentation model switcher `qc_model`, Quantity Detection model switcher `quantity_model`), separate object-detection, QC, and Quantity confidence thresholds, Quantity NMS IoU + class-agnostic merge toggle, defect strategy, defect-class management (grouped enable/add/edit/delete/color), save toast, preferences (language, theme).
...
- **Quantity Detection** (Phase 1: image): dedicated "Quantity" sidebar group with distinct count/history icons. Quantity Detection page runs the configured Quantity Detection model against one or more uploaded images, shows a selected annotated canvas with crop-backed boxes, an image filmstrip, per-image count badges, selected-image object crop evidence with redundant-object delete, session cumulative total + per-class counts derived from remaining crops, Pass/Fail verdict against expected total + tolerance, and saves the corrected result (persisted record + per-image `inputs` crop URLs + `QUANTITY_CHECK` audit log entry with removed count). Quantity History page lists saved checks with search/filter, CSV export, Inspect combined crop gallery, and Delete confirmation.
...
- Quantity Detection (Phase 1: image) endpoints: `POST /api/quantity/detect/image` reuses `object_detection.detect()`/`serialize_detections()` with the dedicated `quantity_model`/`quantity_confidence_threshold` plus Quantity NMS settings (`quantity_nms_iou`, `quantity_agnostic_nms`) (409 if no quantity model configured), writes bbox object crops via `crop_objects()` into `data/quantity/_tmp/<crop_key>/`, returns crop URLs plus each crop's source `box`, serves crops through `GET /api/quantity/crops/{p1}/{p2}/{filename}`, and `POST/GET/GET-by-id/DELETE /api/quantity/checks[/{id}]` persist/list/retrieve/delete saved `QuantityCheck` records (total/per-class counts, expected total, tolerance, verdict, per-image `inputs`) while moving temp crops to `data/quantity/<check_id>/<index>/` and deleting crop folders with checks.
...
│       │   ├── Settings.vue         # Camera CRUD + object/QC/Quantity model/NMS config + defect classes + preferences
...
| Settings | **Functional** | None | Camera CRUD, settings API, grouped "Models" section (object detection / QC-segmentation / Quantity Detection model switchers), separate object/QC/Quantity confidence, Quantity NMS IoU + class-agnostic merge toggle, defect strategy, grouped defect-class enable/add/edit/delete/color, preferences, and save toast wired |
| Quantity Detection | **Functional** (Phase 1: image) | None | Multi-image upload via `/api/quantity/detect/image`, selected annotated canvas, filmstrip switching, crop-backed box overlays, per-image count badges, selected-image crop evidence with redundant-object delete, session-cumulative total + per-class counts from remaining crops, expected-total + tolerance Pass/Fail result band, no-model Settings link, Save toast via `/api/quantity/checks` + `QUANTITY_CHECK` audit log wired; requires a Quantity Detection model configured in Settings |
