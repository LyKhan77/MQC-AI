...
- **Inspect Measurement**: Measurement Studio accepts multiple Image files, registered Live Camera trigger, or HTTPS Mobile Camera capture in one manually named component session. Inspector stages dynamic views, selects one view, chooses dimension/corner/bend task checks, pose (`TOP_FACE`, `REVERSE_FACE`, `PROFILE_FACE`, `CUSTOM_FACE`) and optional Global/Detail profile, draws X/Y calibration, explicitly processes, selects logical edge/corner/bend geometry, sets per-item nominal/tolerance, evaluates `PASS`/`FAIL`/`REVIEW`, saves/reopens/deletes views, and completes only after saved views. OpenCV uses refined LSD/Hough line candidates, logical-edge grouping/fit/dedup, contour arc fitting, bend intersection geometry, and Hough circle candidate + contour/ellipse refinement; physical station accuracy and production profile metrology remain pending.
...
Inspect Measurement is wired at `/measurement`: one manually named session accepts multiple Image/Live/Mobile staged views; selected view task checks, X/Y calibration, and explicit `POST /api/measurements/process` carry pose/profile metadata. The server extracts LSD/Hough candidates, groups/fits/deduplicates logical edges, fits rounded-corner arcs, derives finite-edge bend angles, and returns calibration/task-specific `REVIEW` reasons. Measurement Studio supports dimension/corner/bend multi-select, candidate cards, per-item tolerance, save/reopen/delete, History, and backend Audit events. Physical accuracy, repeatability, station matrix/homography runtime, drawing recipes, and profile metrology remain future gates.
