...
- **Measurement Studio layout**: QC Studio-style fixed-height desktop shell uses wider input/result panels, keeps header and canvas stationary, and gives History and Measurement Items separate internal scroll regions. Canvas uses the same image-frame model as QC Studio for zoom/pan-safe SVG alignment; candidate edges have high-contrast halos, endpoints, and selected measurement labels. On narrow screens, the shell falls back to page scroll so stacked panels remain reachable.
