...
>
> **Direct Inspection:** `/direct-inspection` is the first Quality Control submenu for on-demand QC. Operators can upload images, grab a full-resolution server-camera frame, or explicitly open the mobile browser camera with `getUserMedia({ video: { facingMode: 'environment' } })` over HTTPS. Each capture can use full-frame or model-free auto-crop; an optional debug toggle returns the original frame with the selected crop rectangle and FULL FRAME FALLBACK label. Detection runs through `POST /api/inspection/detect`, previews annotated defect polygons and verdict, can be removed from the stack, then sends `{ key, defects }` captures through `POST /api/inspection/to-qc` to create a finished QC Studio batch with defects already persisted; Re-run and Reset still cover reprocessing or raw review.
>
