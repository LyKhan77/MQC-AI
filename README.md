...
- **Quantity Detection**: dedicated count/verify workflow overlays returned YOLO boxes on a selected uploaded image, provides filmstrip switching, shows selected-image object crop evidence, accumulates session total/per-class counts, computes Pass/Fail against Expected total + Tolerance, tunes NMS IoU/class-agnostic merge to reduce duplicate boxes on one object, saves per-image `inputs` with permanent crop URLs, and provides Quantity History search/filter/CSV/combined-crop Inspect/Delete.
