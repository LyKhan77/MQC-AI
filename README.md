...
**Inspect Measurement current slice:** one manual session can stage multiple Image, Live Camera, and Mobile Camera views. Inspector selects a view, chooses dimension/corner/bend checks, calibrates X/Y, explicitly processes logical OpenCV geometry, selects candidates, evaluates per-item tolerance, saves the view, and completes only after saved views. Global/Detail profiles validate camera/resolution/capability metadata; physical station accuracy and drawing/CAD source-of-truth remain future gates.
