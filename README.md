...
[Live Monitor] Start Camera raw preview
    ...
    OR [Quantity Detection] Add images -> annotated canvas + filmstrip + crop evidence -> delete redundant crops if needed -> verify total verdict -> save check with persisted crop URLs
...
>
> **Quantity Detection Inference Display:** The dedicated Quantity group includes `/quantity` and `/quantity/history`. Quantity Detection uses the configured Quantity Detection model (`quantity_model`), confidence threshold, Quantity NMS IoU (`quantity_nms_iou`), and class-agnostic merge toggle (`quantity_agnostic_nms`). The backend returns one crop record per kept object with its source box; the frontend uses that crop list as the single source of truth for canvas boxes, filmstrip badges, Evidence cards, session total/per-class tally, verdict, and save payload. Inspectors can delete redundant Evidence crops to correct over-detection before saving checks with permanent per-image crop URLs plus a `QUANTITY_CHECK` audit log that includes the removed count. Quantity History lists saved checks, exports CSV, opens Inspect for a combined crop gallery, and deletes checks plus their crop folders through `DELETE /api/quantity/checks/{id}`. Browser smoke needs a real `.pt` selected as the Quantity model and a scene that previously double-counted.
>
...
| `/quantity` | **Quantity Detection** | Multi-image quantity count/verify workflow with selected annotated canvas, image filmstrip, crop-backed box overlays, per-image count badges, selected-image crop evidence delete, result band total/verdict from remaining crops, expected-total/tolerance target inputs, and Save toast |
...
- **Quantity Detection**: dedicated count/verify workflow overlays crop-backed boxes on a selected uploaded image, provides filmstrip switching, shows selected-image object crop evidence, lets inspectors delete redundant crop cards to correct over-detection, derives session total/per-class counts and Pass/Fail from remaining crops, tunes NMS IoU/class-agnostic merge to reduce duplicate boxes on one object, saves per-image `inputs` with permanent crop URLs, and provides Quantity History search/filter/CSV/combined-crop Inspect/Delete.
