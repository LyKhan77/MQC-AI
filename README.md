...
| `/measurement` | **Inspect Measurement** | One-side image or registered Live Camera trigger measurement with manual calibration, OpenCV LSD/Hough edge candidates, per-item tolerance/evaluate, saved History, and Audit Log |
...
- **Inspect Measurement**: Image or registered Live Camera trigger enters one-side Measurement Studio. Inspector supplies a reference pixel length and known millimeters, server OpenCV uses `LineSegmentDetector` first and `HoughLinesP` fallback, candidate edges appear on the canvas, each selected item receives manual nominal/tolerance (linear default `±2.0 mm`), and Evaluate returns `PASS`, `FAIL`, or `REVIEW`. Evaluated runs persist source image, calibration, geometry, History, and measurement audit events. Angle/hole geometry math exists in the kernel; prototype UI focuses on linear edge candidates.
- **Measurement Studio layout**: QC Studio-style fixed-height desktop shell keeps header and canvas stationary; History and Measurement Items use separate internal scroll regions. Mobile Camera is intentionally deferred to the next feedback cycle.
