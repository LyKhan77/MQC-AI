...
[Live Monitor] Start Camera raw preview
    ...
    OR [Inspect Measurement] Image upload / Live Camera trigger -> calibrate -> OpenCV candidate edge -> set per-item tolerance -> evaluate -> save History/Audit
...
| `/measurement` | **Inspect Measurement** | One-side image, registered Live Camera trigger, or HTTPS Mobile Camera capture with manual calibration, OpenCV LSD/Hough edge candidates, per-item tolerance/evaluate, saved History, and Audit Log |
...
- **Inspect Measurement**: Image, registered Live Camera trigger, or HTTPS Mobile Camera capture enters one-side Measurement Studio. Inspector supplies a reference pixel length and known millimeters, server OpenCV uses `LineSegmentDetector` first and `HoughLinesP` fallback, candidate edges appear on the canvas, each selected item receives manual nominal/tolerance (linear default `±2.0 mm`), and Evaluate returns `PASS`, `FAIL`, or `REVIEW`. Evaluated runs persist source image, calibration, geometry, History, and measurement audit events. Angle/hole geometry math exists in the kernel; prototype UI focuses on linear edge candidates.
- **Measurement Studio layout**: QC Studio-style fixed-height desktop shell uses fixed left/canvas/right positioning. The left Input/Calibration rail scrolls independently like the QC batch list, History has its own scroll region, and Measurement Items has its own right-panel scroll region. Canvas uses the same image-frame model as QC Studio for zoom/pan-safe SVG alignment; candidate edges use restrained 3px strokes with contrast halos, endpoints, and selected measurement labels. On narrow screens, the shell falls back to page scroll so stacked panels remain reachable.
