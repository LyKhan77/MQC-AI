...
|---|---|---|
| `qc_frontend/` | **Active** | Vue 3 dashboard dengan 7 halaman, Carbon Design System, i18n bilingual, live `qc_server` API integration, Auto/Manual Live Monitor, annotated MJPEG detection feed/count/FPS, Auto presence-cycle crop results, shared Send-to-QC crop approval gate, Media Detection Test/Process upload page |
...
[Live Monitor] Start Camera raw preview
    ...
    OR [Media Detection] Test upload preview / Process uploaded media to crop review
...
>
> **Media Detection Crop-to-QC:** Settings includes **Enable Media Detection page** (`input_mode_enabled`) to show/hide `/media-detection` in the sidebar. Test mode uploads an image to `POST /api/detect/image` for an annotated base64 result + detection list, or uploads a video to `POST /api/detect/video` and plays `GET /api/detect/video/{id}/stream` as annotated MJPEG. Process mode sends images to `POST /api/detect/image/process` for immediate object crops or uploaded videos to `POST /api/detect/video/{id}/extract` with polling via `GET /api/detect/video/{id}/extract/status`; crop review uses `GET /api/detect/crop-session/{key}`, selected approval uses `POST /api/detect/crop-session/{key}/approve`, then the approved folder is submitted to QC. Uploaded videos are stored under `qc_server/data/uploads/`, which stays gitignored. GPU/model/browser smoke is deferred to the server.
>
...
| `/media-detection` | **Media Detection** | Server-gated upload page with Test preview and Process-to-QC crop export for images/videos |
...
- **Collapsible sidebar navigation** dengan 7 menu items when Media Detection is enabled
...
- **Media Detection page**: gated by `input_mode_enabled`; Test mode uploads sample images/videos to `/api/detect/*` and renders annotated results, while Process mode extracts image/video object crops for shared review and QC batch submission
