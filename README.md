...
**Inspect Measurement current slice:** one manual session can stage multiple Image, Live Camera, and Mobile Camera views. Inspector selects a view, chooses dimension/corner/bend/overall checks, calibrates X/Y, explicitly processes logical OpenCV geometry, selects candidates, evaluates per-item tolerance, saves the view, and completes only after saved views. Geometry kernel v2 uses a Taubin corner fit with arclength windows + tangent exclusion, snaps straight-edge endpoints to virtual corners (mold line), and adds an `outer_dimension` Overall X/Y bounding-box task matching drawing outside dimensions. Global/Detail profiles validate camera/resolution/capability metadata; physical station accuracy and drawing/CAD source-of-truth remain future gates.
...
[Live Monitor] Start Camera raw preview
    ...
    OR [Inspect Measurement] name session -> stage Image/Live/Mobile views -> select view -> calibrate -> explicit Process -> OpenCV line/circle geometry -> tolerance/evaluate -> save view/complete session -> History/Audit
...
| `/measurement` | **Inspect Measurement** | Multi-view Image/Live/Mobile staging with manual series naming, selected-view calibration/process, pose and Global/Detail profile guidance, OpenCV LSD/Hough/circle geometry, per-item tolerance/evaluate, session/view History, and Audit Log |
...
- **Inspect Measurement**: Measurement Studio uses a manual component/series name and dynamic staged views. Image upload accepts multiple files; Live Camera uses `/api/measurements/capture`; Mobile Camera stages client JPEGs. Capture never processes implicitly: inspector selects a view, chooses `TOP_FACE`/`REVERSE_FACE`/`PROFILE_FACE`/`CUSTOM_FACE`, selects optional Global/Detail profile, chooses calibration source, draws calibration, then clicks Process Measurement. Only selected view is sent to OpenCV. Per-item nominal/tolerance (linear default `±2.0 mm`, angle default `±0.5°`) produces `PASS`, `FAIL`, or `REVIEW`; component-edge calibration is review-only, while independent ruler/card calibration is verdict-eligible. Saved views and sessions appear in History and backend Audit. Session/profile endpoints cover CRUD, save/delete/reopen, completion guard, camera/resolution binding, and capability limits. Thickness/bend remain profile-metrology contracts; C50 is development hardware only; station accuracy, drawing recipe, and production calibration remain future gates.
- **Measurement Studio layout**: QC Studio-style fixed-height desktop shell starts immediately below the application TopBar; no in-page heading, outer padding, or modal-like container. Fixed left/canvas/right positioning uses the same `280px / flex / 320px` rail model as QC Studio. The left Input/Calibration rail scrolls independently like the QC batch list, History has its own scroll region, and Measurement Items has its own right-panel scroll region. Source/process/status controls float over the canvas, which uses the same image-frame model as QC Studio for zoom/pan-safe SVG alignment; candidate edges use restrained 3px strokes with contrast halos, endpoints, and selected measurement labels. Current logical edge endpoints are bounded by detected support rather than unrelated global contour extrema; rounded-corner candidates below `1.5 mm` are filtered by default. On narrow screens, the shell falls back to page scroll so stacked panels remain reachable.
