...
>
> **Direct Inspection:** `/direct-inspection` is the first Quality Control submenu for on-demand QC. Operators can upload images, grab a full-resolution server-camera frame, or explicitly open the mobile browser camera with `getUserMedia({ video: { facingMode: 'environment' } })` over HTTPS. Each capture can use full-frame or model-free auto-crop, runs `POST /api/inspection/detect`, previews annotated defect polygons and verdict, can be removed from the stack, then sends `{ key, defects }` captures through `POST /api/inspection/to-qc` to create a finished QC Studio batch with defects already persisted; Re-run and Reset still cover reprocessing or raw review.
>
...
| `/direct-inspection` | **Direct Inspection** | On-demand upload/server-camera/mobile-camera defect check with full-frame or auto-crop, annotated capture stack, remove, and Send to QC Studio as a done batch with persisted defects |
...
- **Direct Inspection**: Quality Control submenu for on-demand defect checks from uploaded images, server-camera frames, or mobile HTTPS camera captures, with full-frame/auto-crop modes, annotated preview stack, remove, and Send to QC Studio as a done batch with persisted defects
